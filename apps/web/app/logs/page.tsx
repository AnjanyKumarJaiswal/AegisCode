...
import {
    Search, Filter, Calendar, Code, Box, Database
} from "lucide-react";
...
export default function AuditLogsPage() {
    ...
    useEffect(() => {
        async function fetchLogs() {
            try {
                const res = await fetch("/api/v2/scan-logs");
                if (res.ok) {
        ...
        <DashboardShell>
            <div className="content-inner">
                ...
                <div className="flex flex-col gap-3">
                    {loading ? (
                        <div className="text-center py-12 text-[#4A4440] font-mono text-xs">Loading...</div>
                    ) : scanLogs.length === 0 ? (
                        <div className="text-center py-12 text-[#4A4440] font-mono text-xs">No scan logs found</div>
                    ) : (
                        scanLogs.map((log) => {
                            const isCritical = log.severity === 'critical';
                            const isHigh = log.severity === 'high';

                            const accentColor = isCritical ? "#D4762A" : isHigh ? "#C4701F" : "#7A9970";
                            const badgeColor = isCritical ? "text-[#D4762A] border-[#D4762A]/20 bg-[#D4762A]/5" :
                                             isHigh ? "text-[#C4701F] border-[#C4701F]/20 bg-[#C4701F]/5" :
                                             "text-[#7A9970] border-[#7A9970]/20 bg-[#7A9970]/5";

                            return (
                                <div key={log.id} className="bg-[#141210] border border-[#2E2A26] rounded-sm relative overflow-hidden group hover:border-[#C4701F]/30 transition-all duration-200">
                                    ...
                                    <div className="flex items-center py-5 px-8">
                                        <div className="w-[30%] flex items-center gap-4">
                                            <div className="w-10 h-10 bg-[#0E0D0C] border border-[#2E2A26] flex items-center justify-center rounded-sm text-[#4A4440] group-hover:text-[#F5F2EE] transition-colors">
                                                {log.severity === 'clean' ? <Database size={18} /> : log.severity === 'high' ? <Box size={18} /> : <Code size={18} />}
                                            </div>
                                            <div className="overflow-hidden">
                                                <h4 className="text-sm font-bold text-[#F5F2EE] tracking-wide m-0 truncate">{log.file}</h4>
                                                <p className="text-[10px] font-mono text-[#4A4440] mt-0.5 truncate">{log.path}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="w-[20%] flex items-center gap-2 text-[11px] font-mono text-[#A89F94]">
                                            {log.engine === 'GitHub Copilot' ? <Box size={12} /> : <Code size={12} />}
                                            {log.engine}
                                        </div>
                                        ...
                                        <div className="w-[20%] flex justify-end">
                                            <div className={`badge-ui ${badgeColor} flex items-center gap-2 min-w-[90px] justify-center`}>
