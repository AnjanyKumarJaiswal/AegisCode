...
import {
    Search, Filter, ChevronRight, ArrowDown, Folder,
    ...
    useEffect(() => {
        async function fetchFindings() {
            try {
                ...
                const res = await fetch(`/api/v2/findings?${params.toString()}`);
        ...
        <DashboardShell>
            <div className="content-inner">
                <div className="flex items-start justify-between mb-12">
                    <div>
                        <h1 className="dash-title !text-5xl !font-bold mb-2">Active Findings</h1>
                        <p className="text-[#4A4440] font-mono text-xs uppercase tracking-widest">
                            {loading ? "—" : findings.length} Total Vulnerabilities Detected Across Infrastructure
                        </p>
                    </div>
                    <div className="flex flex-col gap-3">
                        <div className="flex items-center gap-3">
                            ...
                            <button
                                onClick={() => setSeverityFilter(severityFilter === "all" ? "CRITICAL" : severityFilter === "CRITICAL" ? "HIGH" : severityFilter === "HIGH" ? "MEDIUM" : "all")}
                ...
                <div className="flex flex-col gap-1">
                    {loading ? (
                        ...
                        <div className="text-center py-12 text-[#4A4440] font-mono text-xs">No vulnerabilities found</div>
                    ) : (
                        findings.map((item) => {
                            ...
                            const severityBadgeColor = isCritical ? "text-[#D4762A] border-[#D4762A]/20 bg-[#D4762A]/5" :
                                                    isHigh ? "text-[#C4701F] border-[#C4701F]/20 bg-[#C4701F]/5" :
                                                    isMedium ? "text-[#A89F94] border-[#A89F94]/20 bg-[#A89F94]/5" :
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-center py-6 px-8">
                                        <div className="w-[15%]">
                                            <span className={`badge-ui ${severityBadgeColor} inline-block min-w-[70px] text-center`}>
                                                {item.severity}
                                            </span>
                                        </div>
                                        ...
                                        <div className="w-[20%] flex items-center justify-end gap-6">
                                            ...
                                            <ChevronRight size={18} className="text-[#4A4440] group-hover:text-[#C4701F] transform group-hover:translate-x-1 transition-all" />
                                        </div>
                                    </div>
                                </Link>
                            );
