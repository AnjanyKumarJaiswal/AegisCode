    ...
    async function fetchData() {
      try {
        const [statsRes, feedRes] = await Promise.all([
          ...
          fetch("/api/v2/dashboard/threat-feed"),
        ]);
    ...
    <div className="content-inner">
      ...
      <div className="stats-grid">
        ...
        <div className="card">
          ...
          <div className="flex items-end justify-between mt-8">
            <span className="stat-value-large">
              {loading ? "—" : (stats?.vulnBlocked ?? 0).toLocaleString()}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="threats-section">
        <div className="card !bg-[#141210]">
          ...
          <div className="threat-card-container">
            {loading ? (
              <div className="text-center py-8 text-[#4A4440] font-mono text-xs">
                Loading...
              </div>
                ...
                <div key={idx} className="threat-card-inner">
                  <div>
                    ...
                    <h4 className="text-[14px] font-bold mb-2">
                      {threat.severity} Severity Alert
                    </h4>
                    <p className="text-[11px] text-[#A89F94] font-mono leading-relaxed">
                      Detected in recent scans
                    </p>
                  </div>
          ...
          <tbody>
            {loading ? (
              <tr>
                <td
                  colSpan={4}
                  className="text-center py-8 text-[#4A4440] font-mono text-xs"
                >
                  Loading...
                </td>
              </tr>
